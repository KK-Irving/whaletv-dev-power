---
name: whaletv-commit-message
description: |
  智能 Commit Message 生成（严格五段式 + Branch_Detector 五级降级 + round-trip 契约）。TRIGGERS: "生成 commit message" / "帮我写 commit message" / "commit msg" / "git commit" 前 / push 到 gerrit 前 / PR/CR workflow 走到第 ⑥ 步时. 从三类输入（git diff --staged + Zmind Issue + 目标分支）生成 [版本号][类型][whaletv][Zmind#ID]简述 + [what][why][how][test][impact] 五段。禁止推断 target_version、禁止用 [TBD] 占位、禁止把 subject 里放 ]。Use this skill whenever a commit message is needed. Do NOT use it standalone without Issue context.
---
