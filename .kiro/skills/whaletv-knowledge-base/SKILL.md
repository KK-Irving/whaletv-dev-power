---
name: whaletv-knowledge-base
description: |
  本地知识库（knowledge-mcp）使用规范：三源同步 + BGE-small-zh ONNX 嵌入 + hybrid 检索 + AOSP 模块级精搜 + analyze_issue 端到端。TRIGGERS: "同步知识库" / "重建本地索引" / "找类似 PR" / "search_local" / "跨源检索" / "以前有没有人遇到类似问题" / "找历史 commit". 优先 hybrid mode 默认 source=all。首次启动会自动下 BGE-small-zh ONNX 模型 ~80MB。Use this skill when semantic search over local indexed data is needed. Do NOT use for live API calls (use whaletv-gerrit for real-time gerrit search).
---
