...
name: whaletv-pr-cr
description: |
  PR/CR Issue 端到端处理工作流。TRIGGERS: "处理 PR #12345" / "帮我处理这个 CR" / "PR337540" / "分析这个 PR 然后修" / 任何 "PR/CR/Task + 数字" 且用户期望完整闭环（获取 Issue → 定位代码 → 修改 → git add -p → commit message → 推 Gerrit → 处理评论 → 更新 Zmind）。9 步走完：Issue → 分析 → 定位 → 修改 → 自审 → diff 确认 → add -p → commit msg → push → gerrit-ai 评论 → 更新 Zmind. Use this skill for full PR/CR resolution. Do NOT use if user only wants to read an issue (use zmind get_issue) or only wants a commit message (use whaletv-commit-message).
---
...
2. **本地知识库 `search_local(source="all", mode="hybrid")`**（v2 起 ★ 推荐）：找历史类似 PR、修复 commit、设计文档；命中含相关 gerrit change 时反推改过哪些文件
3. 命中地图后，用路径前缀**限定搜索范围**：
   - 本地：`git grep -n "ClassName" -- "<path-prefix>/**"`（去掉 wrapper 目录前缀，相对源码根）
   - OpenGrok：`search_code` + 结果再筛 path 或 `search_path` 先收敛
4. 未命中地图时，按 local-code-guide 标准 5 档优先级（地图 → 本地索引 → git grep → 已知路径 → OpenGrok）

**预期输出**: 定位到需要修改的文件和代码位置（标注定位方式：module-path-map / search_local / git grep / OpenGrok）
...
**PR/CR 工作流走完或中途停止时，MUST 调用 `generate_report` 生成结构化报告**：

```
generate_report({
  scenario: "pr-cr-resolution",
  task_identifier: "<issue_id>",         // 如 "PR338387"
  skill_name: "whaletv-pr-cr",
  business_summary: {
    title: "<Issue 标题>",
    conclusion: "<修复方案 + gerrit change URL 一句话>",
    details: {
      issue_status: "<处理中 / 已推送 / 已解决>",
      symptom_type: "<症状枚举>",           // 详见 whaletv-bug-analysis Completion Rule
      root_cause_category: "<根因枚举>",     // 详见 whaletv-bug-analysis Completion Rule
      target_version: "<版本号>",
      target_branch: "<推送分支>",
    }
  },
  phases: [
    // 9 步全展开：① 获取 Issue → ② 分析 → ③ 定位 → ④ 修改 → ④.5 自审 → ⑤ 精确暂存 → ⑥ commit msg → ⑦ 推送 → ⑧ gerrit-ai 评论 → ⑨ 更新 Zmind
    // 每步 phase_id + name + status + summary，可含 gate（👤 确认点）
  ],
  artifacts: [
    { type: "gerrit_change_url", value: "<URL>", label: "Gerrit Change" },
    { type: "commit_hash", value: "<sha>" },
    { type: "modified_file", value: "<path>" },
    { type: "zmind_issue_url", value: "<URL>" },
  ],
  hook_metrics: { hooks_triggered: X, hook_names: ["block-git-add-all", ...] },
  final_status: "completed" | "partial" | "gate_blocked" | "aborted",
  output_dir: "<cwd>/report-output"
})
```

**final_status 推断规则**（若不显式传）：
- 所有 9 phase 都 `completed` → `completed`
- 某个 phase `gate_blocked`（如用户拒绝 push） → `gate_blocked`
- 某个 phase `aborted`（如 push 失败） → `aborted`
- 有 `skipped` phase（如用户跳过 verification）但其他都 completed → `partial`

**（可选）上传归档**：如 SoT `s3_issue_analysis` 完整 → `upload_report({ html_path, report_id })`。团队 dashboard 按 ISO 周聚合可见"本周 XX 位 RD 处理了 N 个 PR/CR，60% 是 null_reference 根因"这样的洞察。

**关键约束**：
- 报告生成**不阻塞工作流**：即使 generate_report 失败也不 rollback 前面的 commit / push；只 warn 到 stderr
- 报告是**Skill 执行事实（fact）**：诚实记录跳过 / 阻塞 / 失败的 phase，不美化
