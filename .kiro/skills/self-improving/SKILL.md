...
name: self-improving
description: |
  错误、修正、最佳实践的持续沉淀到 .learnings/（团队知识库）。TRIGGERS: 操作失败时 / 用户纠正 AI 时（"不对"、"应该是..."）/ 发现知识过时或不准确 / API 或工具调用失败. 分类记录到 LEARNINGS.md（correction/knowledge_gap/best_practice）或 ERRORS.md，避免重复踩坑。Use this skill after any failure or user correction. Do NOT skip when a user says "actually, ..." — that's exactly when to record.
---
