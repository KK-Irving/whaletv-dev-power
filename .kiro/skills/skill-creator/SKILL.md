...
name: skill-creator
description: |
  创建和迭代新的 skill（v3 起遵循 .kiro/skills/<name>/SKILL.md 子目录结构 + YAML front-matter with name/description）。TRIGGERS: "创建新 skill" / "写个 skill 处理 xxx" / "capture this as a skill" / "把这个流程做成 skill". 引导用户定义 name + description（含 TRIGGERS + When to Use + When NOT to Use），组装 SKILL.md，可选建 references/ scripts/ assets/ 子目录。Use for creating new skills. Do NOT use for editing existing skills (just str_replace on the SKILL.md).
---
