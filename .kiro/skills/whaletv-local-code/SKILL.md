---
name: whaletv-local-code
description: |
  本地源码搜索 5 档策略（v3 更新）：module-path-map → search_local → git grep → 已知路径直读 → OpenGrok. TRIGGERS: 需要搜索代码 / 定位类名/方法名 / "找一下 XXX 类" / "这个类在哪" / "grep XXX" / Bug 分析走到"定位代码"步骤. git grep 优于 rg（大型代码库 40s → 0.4s），且天然排除 out/prebuilts. Workspace 不含目标代码时提示用户 File → Add Folder to Workspace. Use this skill whenever the AI needs to locate source code. Do NOT use for searching Zmind issues or Gerrit changes (use whaletv-knowledge-base).
---
...
在做任何具体搜索之前，先用 `steering/codebase-taxonomy.md` 确定平台（D4 / X5 / STB）。这直接决定业务代码根是 `vendor/zeasn/`（D4）还是 `vendor/whale/`（X5/STB），影响后面所有搜索的路径前缀。

平台推断方式（自上而下）：
1. **从 Zmind issue.project 读**（如 `cultraview-dvb-amlogic-t950d4-2k-1g` → D4）
2. **从当前 workspace 路径推断**（如 `~/os10_whale/amlogic` 需进一步确认是 X5 还是 STB）
3. **问用户**（如果前 2 步都无法确定）

### ① 模块路径地图查表（最高优先级，零成本）

**首选方式**。确定平台后，从问题描述/异常堆栈中提取关键词（类名、模块名、功能名如 "TvScanConfig"、"TvSettings"、"PQ"、"CEC"、"DTVKit"、"Tuner"），到 `module-path-map.md` **对应平台的小节**中查找路径前缀（**不要跨平台混用路径**）。
