...
description: |
  Zmind 项目标识符 → 本地代码路径的映射管理（用户定制化）。TRIGGERS: "补充配置" / "添加项目映射" / "这个项目对应哪个代码库" / 处理 Issue 时发现项目未映射. 通过 list_projects 展示 Zmind 项目列表，让用户提供本地代码路径，后续处理 Issue 时按 project 字段自动切换 workspace。Use when project ↔ code path mapping needs setup or update. Do NOT use for one-off code searches (use whaletv-local-code).
---
